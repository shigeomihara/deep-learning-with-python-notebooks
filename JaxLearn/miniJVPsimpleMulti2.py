...
def foo_x(x, y):
    return nth_order_derivative(1, lambda x: foo(x, y), x)

print("foo_x(2., 3.)=", foo_x(2., 3.))

def foo_y(x, y):
    return nth_order_derivative(1, lambda y: foo(x, y), y)

print("foo_y(2., 3.)=", foo_y(2., 3.))

# does'n work!
def foo_xy(x, y):
    return nth_order_derivative(1, lambda y: foo_x(x, y), y)

print("foo_xy(2., 3.)=", foo_xy(2., 3.))
